...
namespace Transfers.TaobaoExporters
{
    public class Hishop5_4_2_to_taobao4_6 : ExportAdapter
    {
        ...
        public Hishop5_4_2_to_taobao4_6()
        {
            _exportTo = new TbTarget(ExportVersion);
            _source = new HishopTarget("5.4.2");
        }

        public Hishop5_4_2_to_taobao4_6(params object[] exportParams)
            : this()
        {
        ...
        private string GetProductCSV()
        {
            ...
            foreach (DataRow row in _exportData.Tables["products"].Rows)
            {
                if (row["Description"] != DBNull.Value)
                {
                    ...
                    description = description.Replace("alt=\"\"", "");
                }
                ...
                //Convert.ToDecimal(srows[0]["Price"])
                sb.AppendFormat(productFormat,
                            Trim(Convert.ToString(row["ProductName"])), "0", "0", "0", "", "", "b", skuPrice + Convert.ToDecimal(row["SalePrice"]), "0",
