...
namespace Transfers.TaobaoImporters
{
    public class Hishop5_4_2_from_taobao4_6 : ImportAdapter
    {
        ...
        public Hishop5_4_2_from_taobao4_6()
        {
        ...
        public override object[] ParseProductData(params object[] importParams)
        {
            ...
            using (CsvReader csv = new CsvReader(new StreamReader(Path.Combine(workDir, ProductFilename), System.Text.Encoding.Default), true, '\t'))
            {
                ...
                while (csv.ReadNextRecord())
                {
                    ...
                    {
                        productRow["Description"] = Trim(csv[24].Replace("\"\"", "\""));
                    }
                    ...
                    if (!string.IsNullOrEmpty(pic))
                    {
                        if (pic.EndsWith(";"))
                        {
                            ...
                            {
                                string picName = picArrary[i].Replace(string.Format(":0:{0}:|", i), ".jpg");
                                if (picName.Contains("http:"))
                                    picName = picName.Substring(0, picName.IndexOf("http:"));
                        ...
                        else
                        {
                            ...
                            {
                                File.Copy(Path.Combine(workDir + "\\products", pic.Replace(".jpg", ".tbi")),
                                    context.Request.MapPath("~" + virtualPath + pic), true);
