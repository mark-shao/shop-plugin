...
{
    public class Yfx1_2_from_Taobao4_7 : ImportAdapter
    {
        ...
        public Yfx1_2_from_Taobao4_7()
        {
            _importTo = new YfxTarget("1.2");
            _source = new TbTarget("4.7");
        ...
        public override object[] ParseProductData(params object[] importParams)
        {
            ...
            using (CsvReader csv = new CsvReader(new StreamReader(Path.Combine(workDir, ProductFilename), System.Text.Encoding.Default), true, '\t'))
            {
                ...
                while (csv.ReadNextRecord())
                {
                    ...
                    if (!string.IsNullOrEmpty(csv[25]))
                    {
                        productRow["Description"] = Trim(csv[25].Replace("\"\"", "\"").Replace("alt=\"\"", "").Replace("alt=\"", "").Replace("alt=''", ""));
                    }

                    string pic = Trim(csv[36]);
