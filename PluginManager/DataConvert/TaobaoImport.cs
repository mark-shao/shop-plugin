...
{
    [Plugin("Taobao导入接口")]
    public class TaobaoImport : DataImport
    {
        private int _categoryId;
        private bool _isSell;
        private int _maxSequence;

        public TaobaoImport(int categoryId, bool isSell, int maxSequence)
        {
            ...
            while ((strline = read.ReadLine()) != null)
            {
                ...
                if (i > 1) // 过滤掉标题
                {
                    try
                    {
                        ...
                        //{
                        //    string inFocusImageUrl = AppDomain.CurrentDomain.BaseDirectory + (productToAdd.InFocusImageUrl.Substring(productToAdd.InFocusImageUrl.IndexOf("/") + 1)).Replace("/", @"\");
                        //    string albumImageUrl = inFocusImageUrl.Replace(@"Original\", @"Album\");
                        //    FileInfo fiInFocusImageUrl = new FileInfo(inFocusImageUrl);
                        //    FileInfo fialbumImageUrl = new FileInfo(albumImageUrl);
                        //    if (fiInFocusImageUrl.Exists)
                        //        fiInFocusImageUrl.MoveTo(inFocusImageUrl.Substring(0, inFocusImageUrl.Length - 3) + "jpg");
                        //    if (fialbumImageUrl.Exists)
                        //        fialbumImageUrl.MoveTo(albumImageUrl.Substring(0, albumImageUrl.Length - 3) + "jpg");
                        //    productToAdd.InFocusImageUrl = productToAdd.InFocusImageUrl.Substring(0, productToAdd.InFocusImageUrl.Length - 3) + "jpg";
                        //    productToAdd.ThumbnailsUrl = productToAdd.ThumbnailsUrl.Substring(0, productToAdd.ThumbnailsUrl.Length - 3) + "jpg";
                        //}
                        ...
                        //if (ProductHelper.AddProduct(productToAdd, skus, null, null, null) == ProductActionStatus.Success)
                        //{
                        //    successNum++;
                        //    maxSequence++;
                        //}
