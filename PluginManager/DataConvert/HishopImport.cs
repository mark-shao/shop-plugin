...
{
    [Plugin("Hishop导入接口")]
    public class HishopImport : DataImport
    {
        private int _categoryId;
        private bool _isSell;
        private int _maxSequence;
        public HishopImport(int categoryId, bool isSell, int maxSequence)
        {
            ...
            while ((strline = read.ReadLine()) != null)
            {
                ...
                if (i > 1) // 过滤掉标题
                {
                    try
                    {
                        ...
                        //淘宝格式的数据不能上传
                        if (aryline.Length > 14)
                        {
                            throw new Exception();
                        }

                        ProductInfo productToAdd = null;
                        productToAdd = FillProductInfo(aryline);
